...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
uint8_t led_data[2] = {0x00, 0xff};	// Will be allocated in SRAM
...
int main(void)
{
  ...
  while (1)
  {
	...
	// Transfer data from SRAM to GPIOA peripheral using DMA
	HAL_DMA_Start(&hdma_memtomem_dma2_stream0, (uint32_t)&led_data[0], (uint32_t)&GPIOA->ODR, 1);	
	// Poll for the data transfer to complete (i.e., HAL_DMA_FULL_TRANSFER flag is set)
	HAL_DMA_PollForTransfer(&hdma_memtomem_dma2_stream0, HAL_DMA_FULL_TRANSFER, HAL_MAX_DELAY);
	  
	// Give 1 sec delay
	curr_tick = HAL_GetTick();
	while ((curr_tick + 1000) >= HAL_GetTick());
